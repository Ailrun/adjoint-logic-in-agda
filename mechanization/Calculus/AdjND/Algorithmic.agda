...
module Calculus.AdjND.Algorithmic {ℓ₁ ℓ₂} (ℳ : ModeSpec ℓ₁ ℓ₂) where
open ModeSpec ℳ
...
open import Calculus.AdjND.Typing ℳ
...
data _[_]is-used-by_ : Useable → Mode → Useable → Set ℓ₁ where
  used      : --------------------------
...
data _is-all-used-by_ : Context → ContextDelta → Set ℓ₁ where
  []  : ---------------------
        [] is-all-used-by []
...
data _⦂[_]_∈_⇒_ : ℕ → Mode → Type → Context → ContextDelta → Set ℓ₁ where
  here  : ------------------------------------------------------------
...
data _⊢[_]_⦂_⇒_ : Context → Mode → Term → Type → ContextDelta → Set (ℓ₁ ⊔ ℓ₂) where
  ...
  _⊢`let-return[_⇒_]_⦂_`in_ : Δ ~ Δ₀ ⊞ Δ₁ →
                              m ≤ₘ m₁ →
                              true [ m₀ ]is-used-by d →
                              Γ drop[ m₁ ]⇒ ⊢[ m₁ ] L ⦂ `↓[ m₀ ⇒ m₁ ] T ⇒ Δ₀ →
                              ⊢[ m₁ ] `↓[ m₀ ⇒ m₁ ] T ⦂⋆ →
