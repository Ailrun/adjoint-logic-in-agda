...
true⊢[/]ʳ                                    Δ₀ Γ~ ⊢Γ₁ ⊢L ⊢T (`unit Δ₀dΨ₀Del)
...
true⊢[/]ʳ {M = M `$ N}                       Δ₀ Γ~ ⊢Γ₁ ⊢L ⊢T (Δ₀dΨ₀~ ⊢ ⊢M ⦂ ⊢⊸ `$ ⊢N)
  with ⊢T₀ `⊸[ _ ] _ ← ⊢⊸
...
true⊢[/]ʳ {M = `unlift[ _ ⇒ _ ] M}           Δ₀ Γ~ ⊢Γ₁ ⊢L ⊢T                  (Δ₀dΨ₀∤ ⊢`unlift[-⇒-] ⊢M ⦂ ⊢↑)
...
...      | delete m₁≰ (weakening Wk∈m₀)
        rewrite dec-no (_ ≤?ₘ _) m₁≰
          with Γ₁Del ← ⊢∧Wk≤⇒is-all-del ⊢Γ₁ ≤ₘ-refl Wk∈m₀
             | ⊢M′ ← subst-[/-] eqΔ₀′ M (false⊢[/] _ ⊢↑ ⊢M)                                                             = ~⊞-is-all-del∧⊢⇒⊢ˡ Γ~ Γ₁Del (Δ₀Ψ₀∤ ⊢`unlift[-⇒-] ⊢M′ ⦂ ⊢↑)

true⊢[/]ʳ {M = `return[ _ ⇒ _ ] M}           Δ₀ Γ~ ⊢Γ₁ ⊢L (`↓[-⇒ _ ][ _ ] ⊢T) (Δ₀dΨ₀∤ ⊢`return[-⇒-] ⊢M)
  with _ , _ , refl , Δ₀∤ , d∤ ∷ Ψ₀∤ ← ∤-preserves-++ Δ₀ Δ₀dΨ₀∤
    with Δ₀Ψ₀∤ ← ∤-++⁺ Δ₀∤ Ψ₀∤
       | eqΔ₀′ ← length-respects-∤ Δ₀∤
      with d∤
...
...      | delete m₁≰ (weakening Wk∈m₀)
        rewrite dec-no (_ ≤?ₘ _) m₁≰
          with Γ₁Del ← ⊢∧Wk≤⇒is-all-del ⊢Γ₁ ≤ₘ-refl Wk∈m₀
             | ⊢M′ ← subst-[/-] eqΔ₀′ M (false⊢[/] _ ⊢T ⊢M)                                                             = ~⊞-is-all-del∧⊢⇒⊢ˡ Γ~ Γ₁Del (Δ₀Ψ₀∤ ⊢`return[-⇒-] ⊢M′)

true⊢[/]ʳ {M = `let-return[ _ ⇒ _ ] M `in N} Δ₀ Γ~ ⊢Γ₁ ⊢L ⊢T                  (Δ₀dΨ₀~ ⊢`let-return[-⇒-] ⊢M ⦂ ⊢↓ `in ⊢N)
  with `↓[-⇒ _ ][ _ ] ⊢T₀ ← ⊢↓
     | _ , _ , _ , _ , refl , refl , Δ₀~ , d~ ∷ Ψ₀~ ← ~⊞-preserves-++ Δ₀ Δ₀dΨ₀~
    with Δ₀Ψ₀~ ← ~⊞-++⁺ Δ₀~ Ψ₀~
       | eqΔ₀₀ , eqΔ₀₁ ← length-respects-~⊞ Δ₀~
...
...      | to-left
        with Γ₁′ , Γ₁′~ , Γ~′ ← ~⊞-assocʳ (~⊞-commute Γ~) Δ₀Ψ₀~
          with ⊢M′ ← subst-[/-] eqΔ₀₀ M (true⊢[/]ˡ _ Γ₁′~ ⊢Γ₁ ⊢L ⊢↓ ⊢M)
             | ⊢N′ ← subst-[/-] (cong suc eqΔ₀₁) N (false⊢[/] _ ⊢T ⊢N)                                                 = Γ~′ ⊢`let-return[-⇒-] ⊢M′ ⦂ ⊢↓ `in ⊢N′
...      | to-right
        with Γ₁′ , Γ₁′~ , Γ~′ ← ~⊞-assocˡ Γ~ Δ₀Ψ₀~
          with ⊢M′ ← subst-[/-] eqΔ₀₀ M (false⊢[/] _ ⊢↓ ⊢M)
             | ⊢N′ ← true⊢[/]ʳ _ (to-left ∷ Γ₁′~) ((⊢T₀ , unusable) ∷ ⊢Γ₁) (⊢wk[-↑-] [] (unusable ∷ []) ⊢L) ⊢T ⊢N
            with ⊢N″ ← subst-[/-] (cong suc eqΔ₀₁) N ⊢N′                                                               = Γ~′ ⊢`let-return[-⇒-] ⊢M′ ⦂ ⊢↓ `in ⊢N″
...
...  | yes y≥
    with y ℕ.≟ length Δ₀
...    | yes refl
      with Δ₀Ψ₀Del , refl , refl , refl ← len∈-inversion Δ₀ y∈                                                         = ~⊞-is-all-del∧⊢⇒⊢ʳ Γ~ Δ₀Ψ₀Del ⊢L
...    | no  y≢
